...
if($_POST['title'] && $_POST['message'] && $_GET['section_id']) {
	$sql = "INSERT INTO forum_threads (section_id, title, message, author) VALUES (".$section_id.", '".$title."', '".$message."', '0')";
...
$result = mysql_query($sql)
	or die ("Could not retrieve data from DB! (get threads in section)");
...
while ($row = mysql_fetch_array($result)) {
	...
	$sub_result = mysql_query($sql)
		or die ("Could not retrieve data from DB! (number of answers)");
	...
	if ($row[author] == "0") {
		$author = "<span  class=content_text>Administrator</span>";
	} else {
